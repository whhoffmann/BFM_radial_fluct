...
#new
    def xy_fluctuations_theta(self, dic_ext={}, 
        ...
        ''' Based on theta angle (angle on the plane perpendicular to the rotation plane), 
            ...
            See also: script/bactMotor/radial_fluctuations/hook_bead.odp
        ...
        if 'theta phi fluct' in plots_lev:
            fig7, axs7 = plt.subplot_mosaic('''abcde\nfglhi\nmnopq''', num='xy_fluctuations 7', clear=True)
        if 'xyz' in plots_lev:
        ...
        filter_win   = 801                  # filter pts for speed_Hz_f 
        plotlist3    = [1,15,30]            # windows to plot in fig3
        # init:
        thetavar_sig_arr  = []
        theta_stds_loren  = []
        speedmn_arr       = []
        ...
        dwt_var_arr         = []
        L_arr               = []
        dist_beadcent_wall_arr     = []
        dist_beadcent_wall_err_arr = []
        # start analysis:
        ...
        if correct_drift_xy or correct_drift_z:
            # find drift file automatically and correct drift in xyz: (errors?) 
            try:
                if correct_drift_xy:
        ...
        x = x - np.mean(x)
        y = y - np.mean(y)
        angle_turns = np.unwrap(np.arctan2(y, x))/(2*np.pi)     # only used in z correct_sig_modulation() below
        x_orig, y_orig = x,y
        ...
        # case xyz, define theta from xyz,offset:
        ...
        # CW-CCW: threshold speed, remove where abs(speed) < speed_thr:
        if speed_thr:
            ...
            if hasattr(self, 'z'): 
                z = z[idx]
        # CW-CCW: cut and append CW parts at the end of speed trace:
        ...
        for i, di in zip(idxs, didxs):
            ...
            yw = y[i:i+di]
            if center_win_traj:
                # center xy trajectory to origin for each win:
            ...
            if beta!=None and Dbeta!=None:
                ...
                radw = radw[betaidxs]
            # use histogram of radius to find rmax:
            foo, radbins[k] = np.histogram(radw, 40, density=False)
            k = k+1
        # as the rmax of each win is similar (the min increases more than the max), take the median of the rmax of all the win:
        rmax = np.median([radbins[k][-1] for k in radbins.keys()])
        L = np.sqrt((offset + r_bead)**2 + rmax**2)         # origin to bead-center. offset is inserted here.
        ...
        for i,di in zip(idxs, didxs):
            ...
            # windowing:
            xw = x[i:i+di]                                  # x in win
            yw = y[i:i+di]                                  # y in win
            ...
            speedw_Hz = speed_Hz[i:i+di]                    # motor speed omega in win
            speedw_Hz_f = speed_Hz_f[i:i+di]                # motor speed omega in win, filtered 
            ...
            # define theta in win, case xyz:
            if hasattr(self, 'z') and use_xyz:
                zw = z[i:i+di]
                thetaw = theta[i:i+di]                        # hook angle theta, case xyz
                if 'ztheta' in plots_lev:
            ...
            # case xy:
                ...
                thetaw = np.arccos(np.clip(radw/L, -1, 1))    # hook angle theta, case xy
            ...
            radstd_arr        = np.append(radstd_arr      , np.std(radw))
            ...
            if hasattr(self, 'z') and use_xyz:
                ...
                # propagate error on dist_beadcent_wall:
                L_err = 0 # TODO
                radw_mn = np.mean(radw)
                radw_err = np.std(radw)   
                dist_beadcent_wall_mn = np.mean(dist_beadcent_wall)
                dist_beadcent_wall_err = np.sqrt((L/dist_beadcent_wall_mn)**2 * L_err**2 + (radw_mn/dist_beadcent_wall_mn)**2 * radw_err**2)
                LL = L
                if np.any(dist_beadcent_wall <= r_bead): 
                    print('xy_fluctuations(): Warning! dist_beadcent_wall <= r_bead')
            if 'xyz' in plots_lev:
            ...
            dist_beadcent_wall_arr     = np.append(dist_beadcent_wall_arr    , np.mean(dist_beadcent_wall))
            dist_beadcent_wall_err_arr = np.append(dist_beadcent_wall_err_arr, dist_beadcent_wall_err)
            # Calculate angular Drag Coeff. (theta, plane r,z), combining Brenner's corrections:
            with warnings.catch_warnings():
                warnings.simplefilter('error')
                gamma0, gamma_parallel_faxen, gamma_perp_faxen, gamma_parallel_brenner, gamma_perp_brenner = Drag.DragTranslationBead.calc_drag(r_bead, dist_beadcent_wall, prints=False)
            _gamma_theta_brenner = np.mean(LL**2 * (gamma_perp_brenner * np.cos(thetaw)**2 + gamma_parallel_brenner * np.sin(thetaw)**2))
            _gamma_theta_faxen   = np.mean(LL**2 * (gamma_perp_faxen   * np.cos(thetaw)**2 + gamma_parallel_faxen   * np.sin(thetaw)**2))
            _gamma_theta_max = np.max([_gamma_theta_brenner, _gamma_theta_faxen]) 
            gamma_theta = np.append(gamma_theta, _gamma_theta_max)
            if np.argmax([_gamma_theta_brenner, _gamma_theta_faxen]) == 1:
                print(f'\nxy_fluctuations_theta(): Note. gamma_theta chosen from Faxen') 
            ...
            if 3 in plots_lev:
                ...
                ax41.set_xlabel('idx')
                ax41.set_ylabel('hypot(xw,xw)')
                ax42.plot(radw + 8*k*np.std(radw), '.', ms=1)
                ax42.set_xlabel('idx')
                ax42.set_ylabel('radw')
                ax43.plot(np.mod(tanw, 2*np.pi), np.hypot(xw, yw) + 8*k*np.std(radw), '.', ms=1)
                ax43.set_xlabel('tanw mod 2pi')
                ax43.set_ylabel('hypot(xw,xw)')
                ax44.plot(np.mod(tanw, 2*np.pi), radw + 8*k*np.std(radw), '.', ms=1)
                ax44.set_xlabel('tanw mod 2pi')
                ax44.set_ylabel('radw')
            if 4 in plots_lev:
                self.allan_variance(radw, n_taus=20, FPS=self.FPS, plots=True, clear=False)
            if 'theta phi fluct' in plots_lev:
                axs7['a'].plot(np.median(speedw_Hz_f), np.var(thetaw), 'ko', alpha=0.3)
                axs7['a'].set_xlabel(r'$\omega$ (Hz)')
                axs7['a'].set_ylabel(r'$\sigma^2_{\theta}$ (rad$^2$)')
                axs7['b'].plot(np.median(speedw_Hz_f), np.var(speedw_Hz), 'ko', alpha=0.3)
                axs7['b'].set_xlabel(r'$\omega$ (Hz)')
                axs7['b'].set_ylabel(r'$\sigma^2_{\omega}$ (Hz$^2$)')
                axs7['c'].plot(np.median(speedw_Hz_f), np.var(radw*1e9), 'ko', alpha=0.3)
                axs7['c'].set_xlabel(r'$\omega$ (Hz)')
                axs7['c'].set_ylabel(r'$\sigma^2_{r}$ (nm$^2$)')
                ...
                axs7['d'].loglog(np.median(speedw_Hz), np.var(dwt), 'ko', alpha=0.3)
                axs7['d'].set_xlabel(r'$\omega$ (Hz)')
                axs7['d'].set_ylabel(r'$\sigma^2_{dwt}$ ($s^2$)')
                # stepsize from randomness parameter:
                stepsize = np.var(dwt)*np.median(speedw_Hz*2*np.pi)**2/(dwt_win*np.pi/180) # rad
                axs7['e'].plot(np.median(speedw_Hz), 2*np.pi/stepsize, 'ko', alpha=0.3)
                axs7['e'].set_xlabel(r'$\omega$ (Hz)')
                axs7['e'].set_ylabel(r'Steps / Turn')
                axs7['f'].plot(np.var(speedw_Hz), np.var(thetaw), 'ko', alpha=0.3)
                axs7['f'].set_xlabel(r'$\sigma^2_{\omega}$ (Hz$^2$)')
                axs7['f'].set_ylabel(r'$\sigma^2_{\theta}$ (rad$^2$)')
                axs7['g'].plot(np.var(speedw_Hz), np.var(radw), 'ko', alpha=0.3)
                axs7['g'].set_xlabel(r'$\sigma^2_{\omega}$ (Hz$^2$)')
                axs7['g'].set_ylabel(r'$\sigma^2_{r}$')
                axs7['l'].plot()
                axs7['h'].plot(np.var(speedw_Hz), np.var(dwt), 'ko', alpha=0.3)
                axs7['h'].set_xlabel(r'$\sigma^2_{\omega}$ (Hz$^2$)')
                axs7['h'].set_ylabel(r'$\sigma^2_{dwt}$ (s$^2$)')
                axs7['i'].plot(np.var(thetaw), np.var(dwt), 'ko', alpha=0.3)
                axs7['i'].set_xlabel(r'$\sigma^2_{\theta}$ (rad$^2$)')
                axs7['i'].set_ylabel(r'$\sigma^2_{dwt}$ (s$^2$)')
        ...
        self.radstd_arr                 = radstd_arr
        self.L_arr                      = L_arr
        self.dist_beadcent_wall_arr     = dist_beadcent_wall_arr
        self.dist_beadcent_wall_err_arr = dist_beadcent_wall_err_arr
        
        # Mean Square Diff (log) between gamma_theta and gamma_loren_f2, to minimize by the best choice of 'offset':
        MSE_gamma_loren_brenner = np.log10(np.mean((gamma_loren_f2 - gamma_theta)**2))
        print(f'\nxy_fluctuations(): MSE_gamma_loren_brenner: {MSE_gamma_loren_brenner:.3f} with offset:{offset*1e9:.3f} nm')
        print(f'xy_fluctuations(): Done in {time.time()-t0:.1f} sec.')
        print()
        ...
        if 'theta phi fluct' in plots_lev:
            try:
                ...
                axs7['d'].plot(px, 10**po(np.log10(px)), '--', alpha=0.5)
                axs7['d'].set_title(f'dwelltimes in {dwt_win}°, slope:{pz[0]:.1f}', fontsize=8)
            except:
            ...
            axs7['f'].plot(np.linspace(speedvar_arr.min(), speedvar_arr.max(), 100), po(np.linspace(speedvar_arr.min(), speedvar_arr.max(), 100)), '--')
            axs7['m'].plot(x[::50], y[::50], ',', alpha=0.1)
            axs7['n'].plot(thetamn_sig_arr, '.')
            axs7['n'].set_xlabel('win#')
            axs7['n'].set_ylabel(r'$\langle \theta \rangle$ (rad)')
            axs7['o'].plot(speedmn_arr, '.')
            axs7['o'].set_xlabel('win#')
            axs7['o'].set_ylabel(r'$\omega$ (Hz)')
            fig7.tight_layout()
        if 1 in plots_lev:
            ...
            #ax13.plot(speedmn_arr, gamma_loren_f2_pts/(gamma*L_arr**2), 'g.', ms=3, alpha=0.8, label='Lorentz_f2_pts')
            #ax13.plot(speedmn_arr[idx_keep], gamma_loren[idx_keep]/(gamma*L**2), 'xr', ms=4, alpha=0.8, label='keep')
            ax13.plot(speedmn_arr, gamma_theta/(gamma*L_arr**2), 'b.', ms=3, alpha=0.8, label='Brenner')
            ...
            #ax15.plot(speedmn_arr[idx_loren], MSE_loren[idx_loren], 'rx', ms=4, alpha=0.5)
            ...
            #ax19.semilogy(speedmn_arr[idx_gauss], np.array(list(thetadists_popt_err.values()))[idx_gauss], 'rx', ms=4, alpha=0.5)
            ...
            ax23.plot(gamma_theta/(gamma*L_arr**2), 'b.', ms=3, alpha=0.8, label='Brenner')
